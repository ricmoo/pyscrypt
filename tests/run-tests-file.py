...
for text_length in [3, 16, 127, 128, 129, 1500]:
    ...
    fp = StringIO()
    sf = ScryptFile(fp = fp, password = b"password", N = 1024, r = 1, p = 1, mode = ScryptFile.MODE_WRITE)
    sf.write(plaintext)
    sf.finalize()
    fp.seek(0)
    ...
    fp = StringIO(crypttext)
    sf = ScryptFile(fp = fp, password = b"password", mode = ScryptFile.MODE_READ)
    decrypted = sf.read()
    sf.close()
...
for length in (10, 100, 1000):
    ...
    text = b"Hello world" * length
    sf = ScryptFile(file(path_scrypt, ScryptFile.MODE_WRITE), b"password", 1024, 1, 1)
    sf.write(text)
    sf.close()
...
for filename in ('test1', 'test2'):
    ...
    valid = ScryptFile.verify_file(file(path_scrypt, ScryptFile.MODE_READ), b'password')
    result = {True: "pass", False: "fail"}[valid]
    print("Test Verify: filename=%s result=%s" % (path_scrypt, result))

    for test in (0, 1, 2):
        sf = ScryptFile(file(path_scrypt, ScryptFile.MODE_READ), b'password')
        f = file(path_text, ScryptFile.MODE_READ)
